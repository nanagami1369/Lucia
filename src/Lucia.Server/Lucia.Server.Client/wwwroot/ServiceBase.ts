...
export abstract class ServiceBase {
    ...
    private currentSentinel: CleanupSentinel | undefined;

    /**
     * 派生クラスで実装: 要素へのイベントリスナー登録など
     *
     * @param target 対象のHTML要素
     * @param args mount()から渡された追加引数
     * @returns 登録成功時は true、失敗時は false
     */
    protected abstract register(target: HTMLElement, ...args: any[]): boolean;

    /**
     * 派生クラスで実装: イベントリスナーの解除など
     *
     * 注意：このメソッドはDOM削除時に自動的に呼ばれる
     * 手動で呼ぶ必要はない（unmount()を使う場合を除く）
     */
     ...
     */
    mount = (target: HTMLElement | string | unknown | undefined, ...args: any[]): void => {
        ...
        const sentinel = document.createElement(CLEANUP_SENTINEL_TAG) as CleanupSentinel;
